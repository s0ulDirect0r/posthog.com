...
If a cloud task didn't link its PR, the link is extracted from the PR URL in the agent's tool output. If the agent opened the PR via an unusual path that hides the URL, the link may not be auto-attached, but the PR will still exist on GitHub.
